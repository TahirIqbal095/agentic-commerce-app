...
test("Customer adds a recommended Product without optimistic Cart changes or model interpretation", async (t) => {
        ...
        transcript: [
          {
            ...
            customerMessage: "Show me earphones",
            result: {
              status: "COMPLETED",
              conversationId: "41000000-0000-4000-8000-000000000001",
  ...
  const add = view.getByRole("button", { name: "Add Quiet Buds to Cart" });
  ...
  const command = JSON.parse(String(requests[0]?.[1]?.body));
  assert.equal(command.type, "ADD_PRODUCT");
  assert.equal(command.productId, productId);
  assert.match(command.mutationKey, /^[0-9a-f-]{36}$/);
...
test("Customer uses the same deterministic Add behavior from Product details", async (t) => {
  ...
  await user.click(
    view.getByRole("button", { name: "View Quiet Buds details" }),
  );
