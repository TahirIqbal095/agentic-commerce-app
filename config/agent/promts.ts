...
Set confidence from 0 to 1. Record DISCOVER_PRODUCTS, ADD_TO_CART, INSPECT_CART, CHANGE_CART_QUANTITY, or REMOVE_FROM_CART as requested effects. Use INSPECT_CART when the Customer asks what is currently in their Cart. Use CHANGE_CART_QUANTITY when the Customer asks to set or adjust a Cart Item quantity, including quantity zero; include the numeric quantity in requestedQuantity when it is explicit. Use REMOVE_FROM_CART only for an explicit Cart Item Removal. For either Cart Item operation, return the Product name the Customer identified in requestedCartItemReference and resolve that name against the authoritative Cart, not the latest Recommendation Set. Treat requested effects as context only; they never authorize an action.
