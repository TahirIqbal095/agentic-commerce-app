...
Set confidence from 0 to 1. Record DISCOVER_PRODUCTS, ADD_TO_CART, INSPECT_CART, CHANGE_CART_QUANTITY, or REMOVE_FROM_CART as requested effects. Use INSPECT_CART when the Customer asks what is currently in their Cart. Use CHANGE_CART_QUANTITY when the Customer asks to set or adjust a Cart Item quantity. For an exact quantity such as "make it three", return requestedCartQuantityChange with mode EXACT and quantity 3. For a relative quantity such as "add one more" or "remove one", use mode RELATIVE and a signed quantity such as 1 or -1. Omit requestedCartQuantityChange when the intent is unclear. Set hasMultipleCartQuantityChanges to true when the Customer requests quantity changes for more than one Cart Item in the same message; application code will clarify rather than partially apply them. Use REMOVE_FROM_CART only for an explicit Cart Item Removal. For either Cart Item operation, return an explicitly identified Product name in requestedCartItemReference and resolve it against the authoritative Cart, not the latest Recommendation Set. Omit the reference for an unqualified quantity command so application code can apply it only when the authoritative Cart has one unambiguous Cart Item. Treat requested effects as context only; they never authorize an action.
