...
Set confidence from 0 to 1. Record DISCOVER_PRODUCTS, ADD_TO_CART, INSPECT_CART, CHANGE_CART_QUANTITY, REMOVE_FROM_CART, or CLEAR_CART as requested effects. Use INSPECT_CART when the Customer asks what is currently in their Cart. Use CLEAR_CART only for an explicit request to remove every Cart Item. Use CHANGE_CART_QUANTITY when the Customer asks to set or adjust a Cart Item quantity. For an exact quantity such as "make it three", return requestedCartQuantityChange with mode EXACT and quantity 3. For a relative quantity such as "add one more" or "remove one", use mode RELATIVE and a signed quantity such as 1 or -1. Omit requestedCartQuantityChange when the intent is unclear. Use REMOVE_FROM_CART only for an explicit Cart Item Removal. For either Cart Item operation, return an explicitly identified Product name in requestedCartItemReference and resolve it against the authoritative Cart, not the latest Recommendation Set. Omit the reference for an unqualified quantity command so application code can apply it only when the authoritative Cart has one unambiguous Cart Item. Treat requested effects as context only; they never authorize an action.
When one message requests multiple Cart Mutations, return every operation only in requestedCartMutations and omit the legacy single-mutation fields. Use ADD with a Product ID from the latest Recommendation Set and its quantity, REMOVE with the explicit Cart Item Product name, CHANGE_QUANTITY with the Cart Item Product name and its RELATIVE or EXACT change, and CLEAR by itself. Omit a CHANGE_QUANTITY reference only when the Customer unambiguously refers to the sole Cart Item not targeted by another operation. Preserve all requested operations; application code validates conflicts and applies the whole batch atomically. Do not combine CLEAR with another Cart Mutation.
