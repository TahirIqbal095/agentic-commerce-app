...
export function createLegacyCommerceAgent(
  ...
  conversation: LegacyConversationModule | ConversationModule,
): LegacyCommerceAgent {
  return {
    async respond(input): Promise<AgentResponse> {
      ...
      await (turn.complete as (message: string) => Promise<void>)(response.message);
      return { ...response, conversationId: turn.conversationId };
    },
  };
}

function toCatalogSearch(intent: ShoppingIntent) {
  return {
